...
func (s *mockDecoServer) handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	if strings.Contains(path, "/login") || strings.Contains(query, "form=keys") || strings.Contains(query, "form=auth") || strings.Contains(query, "form=login") {
		...
		if form == "" {
			// Parse from query string which may be after semicolon in path
...
func (s *mockDecoServer) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	// Decrypt signature to get AES key/IV
	...
	// Encrypt response with client's AES key
...
func (s *mockDecoServer) handleAPI(w http.ResponseWriter, r *http.Request, path, query string) {
	...
	reqJSON, err := aesDecrypt(encData, s.aesKey, s.aesIV)
	if err != nil {
	...
	json.Unmarshal(reqJSON, &reqData)
...
func TestGetClients(t *testing.T) {
	...
	// Should only have online clients (2 out of 3)
	if data.Count != 2 {
	...
	// Verify first client (sorted by IP, .100 comes first)
	c := data.Clients[0]
	if c.Name != "TestPhone" {
...
func TestValidateConfig(t *testing.T) {
	// Test with a server that's running
	...
	config2 := &Config{Host: "192.0.2.1:1", Password: "test"} // RFC 5737 TEST-NET
...
func TestFilterClients(t *testing.T) {
	data := &ClientList{
		Clients: []ClientInfo{
			{Name: "XBOX", IP: "192.168.68.71", MAC: "68-6C-E6-F2-75-5B", Connection: "WiFi 5GHz"},
			{Name: "TestPC", IP: "192.168.68.101", MAC: "11-22-33-44-55-66", Connection: "Wired"},
			{Name: "Roku", IP: "192.168.68.61", MAC: "00-BF-AF-C2-98-2F", Connection: "WiFi 2.4GHz"},
		},
		Count: 3,
	}

	t.Run("filter by name", func(t *testing.T) {
		result := filterClients(data, "xbox", "")
		if result.Count != 1 {
			t.Errorf("Count = %d, want 1", result.Count)
		}
		if result.Clients[0].Name != "XBOX" {
			t.Errorf("Name = %q, want %q", result.Clients[0].Name, "XBOX")
		}
	})

	t.Run("filter by name case insensitive", func(t *testing.T) {
		result := filterClients(data, "TESTPC", "")
		if result.Count != 1 {
			t.Errorf("Count = %d, want 1", result.Count)
		}
	})
	...
	t.Run("filter by mac", func(t *testing.T) {
		result := filterClients(data, "", "11-22-33-44-55-66")
		if result.Count != 1 {
			t.Errorf("Count = %d, want 1", result.Count)
		}
	})

	t.Run("filter by mac case insensitive", func(t *testing.T) {
		result := filterClients(data, "", "11-22-33-44-55-66")
		if result.Count != 1 {
			t.Errorf("Count = %d, want 1", result.Count)
		}
	...
	t.Run("no match", func(t *testing.T) {
		result := filterClients(data, "nonexistent", "")
		if result.Count != 0 {
			t.Errorf("Count = %d, want 0", result.Count)
		}
	})

	t.Run("combined name and mac", func(t *testing.T) {
		result := filterClients(data, "xbox", "68-6C-E6-F2-75-5B")
		if result.Count != 1 {
			t.Errorf("Count = %d, want 1", result.Count)
		}
	})

	t.Run("combined name and wrong mac", func(t *testing.T) {
		result := filterClients(data, "xbox", "00-00-00-00-00-00")
		if result.Count != 0 {
			t.Errorf("Count = %d, want 0", result.Count)
		}
	})
...
func TestAliasLoadSave(t *testing.T) {
	...
	aliasPath := tmp + "/deco_aliases.json"
	os.WriteFile(aliasPath, []byte(`{"AA-BB-CC-DD-EE-FF":"My Phone","11-22-33-44-55-66":"My PC"}`), 0644)
	...
	if aliases["AA-BB-CC-DD-EE-FF"] != "My Phone" {
		t.Errorf("alias = %q, want %q", aliases["AA-BB-CC-DD-EE-FF"], "My Phone")
	}
}
